...
if not API_KEY or len(API_KEY) < 20:
    st.error("🛑 系統沒有讀到金鑰！請確認 Secrets 設定。")
    st.stop()

# ==========================================
# 3. 初始化與 API 設定 (新版套件 + 強制 Header 避開 AQ 誤判)
# ==========================================
...
if not os.path.exists(CSV_FILE):
    with open(CSV_FILE, mode='w', encoding='utf-8-sig', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            '時間', '前測_過去AI使用頻率', '前測_過去AI滿意度', '前測_對AI的信任度',
            '情境前測內容', '系統分配屬性', '互動次數',
    ...
    if st.button("開始進入系統") and user_input:
        with st.spinner('系統設定中...'):
            ...
            if "高" in result:
                ...
                system_instruction = "你現在是一個「高語境」的聊天機器人。你的核心目標是創造『社會親近感』。\n1. 關係導向：展現溫暖、同理心，像朋友一樣聊天。\n2. 語氣自然：多使用口語化詞彙，並適當加入 Emoji。\n3. 重視情境：用對話的方式引導，讓使用者覺得「你懂他」。"
            else:
                st.session_state.context_style = "Low-Context"
                system_instruction = "你現在是一個「低語境」的聊天機器人。你的核心目標是創造『訊息清晰度』。\n1. 任務導向：直接、明確、精準，不說廢話。\n2. 語氣專業：不要使用表情符號(Emoji)，不要有多餘的情感寒暄。\n3. 資訊完整：務必使用「條列式」或「表格」來整理重點，追求最高效率的資訊傳遞。"
            ...
            if submit_btn:
                with open(CSV_FILE, mode='a', encoding='utf-8-sig', newline='') as f:
                    ...
                    writer.writerow([
                        ...
                        st.session_state.pre_freq, st.session_state.pre_sat, st.session_state.pre_trust,
                        st.session_state.onboarding_text, st.session_state.context_style,
